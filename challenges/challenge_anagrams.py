def is_anagram(first_string, second_string):
    if len(first_string) == 0 and len(second_string) != 0:
        ...
        sorted_second_string = merge_sort(
            second_string_letters, 0, len(second_string_letters))
        return (
            first_string,
            sorted_second_string,
            first_string == sorted_second_string)

    elif len(second_string) == 0 and len(first_string) != 0:
        first_string_letters = list(first_string.lower())
        sorted_first_string = merge_sort(
            first_string_letters, 0, len(first_string_letters))
        return (
            sorted_first_string,
            second_string,
            sorted_first_string == second_string)
